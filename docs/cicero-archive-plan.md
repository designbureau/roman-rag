# Roman Archive — Project Plan & Handover
...
- ✅ **`chat` deployed + verified live** (Edge Function `chat` v1, ACTIVE; `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` set as Edge secrets). §5 verification passed against the live corpus: `cicero` answers the death-of-Lucius query in first person grounded in `Att. 1.5`, quoting the real Latin (`quantum dolorem acceperim…`) — genuine parallel text; refuses to cross its death-bound ("I cannot speak of what came after me"); `classicist` is candid about transmission/translation and keeps primary text distinct from the biographical tradition.
- ✅ **Auth disabled for the slice** — `web/app/lib/config.ts` `AUTH_ENABLED = false` (the site is open; the chat panel always sends the anon key so chat works without sign-in; `/admin` stays gated/disabled pending the auth layer). `web/.env.local` (gitignored) holds the `VITE_*` vars so `pnpm build:web` / `pnpm dev` run; SPA build verified green.
- ✅ **Phase-3 voices written (code) — `tiro`, `atticus`, `caesar`, `interpreter`, `storyteller`.** Added as code built-ins in `supabase/functions/chat/index.ts` alongside classicist/cicero (now seven; `BuiltinPersona` union + `PERSONA_PROMPTS` extended). Each follows the existing identity/VOICE/bounds/REFUSAL + `SHARED_RULES` shape: `tiro` (the recorder; OUTLIVED Cicero, so may speak the meta-frame of how the letters were collected/published); `atticus` (the addressee; urbane confidant, 110–32 BC, outlived Cicero); `caesar` (HARD-bound at the Ides of March 44 BC — refuses the Philippics, his assassination, the proscriptions, Cicero's death, the Empire); `interpreter` (plain-English, no persona/padding); `storyteller` (temp 0.8, accessible retelling, three code age-tiers young/standard/teen in `CODE_AGE_TIERS`). Frontend wired: `FALLBACK_PERSONAS` (persona-toggle.tsx) + `PERSONA_NAMES` (citations-panel.tsx) now list all seven. **Web SPA build green.** Deno typecheck not run locally (deno absent); template literals balanced + deploy compiles on upload.
- ✅ **Deferred migrations written (repo) — `profiles`/`handle_new_user` + Roman `persona_config`.** New `20260617000000_profiles.sql` creates the missing `profiles` table (id→auth.users, `is_admin`) + `handle_new_user()` signup trigger (SECURITY DEFINER, search_path pinned, EXECUTE revoked from API roles) + own-row RLS that blocks self-escalation to admin. `20260618000000_dynamic_personas.sql` re-seeded to the **Roman ensemble** (classicist/cicero/tiro/atticus/caesar/interpreter/storyteller) — the |xam Early Race seed + Bleek-Lloyd built-ins dropped. `20260618120000_persona_age_tiers.sql` is corpus-agnostic and now seeds the real `storyteller` (kept as-is). `language_groups` left **deferred** (single-language Latin corpus; rewriting the `search_chunks` signature for a no-op filter isn't worth the coupling yet).
- ✅ **Phase-3 applied + deployed live + verified (`zypnwehtzzwxlepyrbjh`).** Three migrations applied via Supabase MCP `apply_migration` in order — `profiles` (profiles table + `handle_new_user` trigger + own-row RLS), `dynamic_personas` (persona_config table + RLS + Roman ensemble seed: 7 built-ins + `__shared__`), `persona_age_tiers` (storyteller seeded with 3 tiers). `chat` **redeployed to v2** (ACTIVE, `verify_jwt`=true preserved) via the **Supabase CLI** (`supabase functions deploy chat --project-ref …`, reads from disk — fidelity-safe, no inline transcription). §5 live verification passed against the live corpus: `caesar` refuses the Philippics ("that day closed my account"); `tiro` speaks the post-Cicero meta-frame (letters gathered/published by Atticus, copying through the household) grounded in real retrieved lines; `interpreter` plain-renders and refuses when the target passage wasn't retrieved; `storyteller` refuses to invent and offers only what was retrieved. Frontend (`web`) runs locally at :5173 against the live backend (`web/.env.local` written with the live URL + legacy anon JWT; the `sb_publishable_` key is NOT a JWT and fails `verify_jwt`).
- ✅ **Ensemble trimmed to FIVE voices — `interpreter` and `storyteller` removed** (user decision). Final built-ins: `classicist`, `cicero`, `tiro`, `atticus`, `caesar`. Removed from `chat/index.ts` (`PERSONA_PROMPTS`/`BuiltinPersona`), `persona-toggle.tsx` `FALLBACK_PERSONAS`, `citations-panel.tsx` `PERSONA_NAMES`, the `dynamic_personas` seed, and the live `persona_config` table; the orphaned storyteller age-tier seed neutralised (the `age_tiers` column + machinery stay for admin-authored personas). `chat` redeployed (v4) via CLI. Live toggle verified: 5 voices, no interpreter/storyteller.
- ✅ **Layer 5 — frontend re-theme.** Identity re-pointed Bleek-Lloyd → Cicero across the app: `root.tsx`/`_index.tsx` (title, h1, blurb, footer sources → Perseus / Tyrrell–Purser Latin / Shuckburgh English), `site-nav.tsx` (trimmed to the sections with real data — chat, library, papers — with topics/graph/heatmap/glossary unlinked until their data lands), `filter-bar.tsx` (informant→**Addressee**, source→**Collection**, populated with the real single facet: Atticus / Letters to Atticus), and the analytics/papers routes + components + libs + three new Cicero papers (via subagent). **Palette:** the single accent shifted terracotta → **imperial/Tyrian purple** (`--color-tyrian #7a2e57` / `-strong #5b1f40`) over the same warm-stone, serif, zero-shadow aesthetic. Fixed a latent bug: the default-persona was still `archivist` in `_index.tsx` (storage default + URL-strip) → now `classicist`.
- ✅ **Layer 4 — analytics, as far as the corpus + keys allow.** `/library` **regenerated for real** from the live DB (460 letters grouped into the 16 books of ad Atticum, with snippets/refs/addressee). `topics/build.ts` **rewritten for Cicero** (typecheck-clean; reads `data/stories.json`, builds the book-grouped library, emits honest empty stubs for the theme-driven datasets) so `pnpm scrape && pnpm topics` reproduces the library locally with no keys. The theme-driven analytics JSON (`topics`, `topics-graph`, `topics-heatmap`, `glossary`, `glossary-lookup`) replaced with **empty Cicero stubs** so those routes show clean empty states instead of leaking |xam data.
- ⏭ **BLOCKED on Layer-4 completion — needs API keys + fuller corpus.** The `themes[]` LLM theme-tagging pass (feeds /topics, /graph, /heatmap) and the Latin glossary (Lewis & Short + Smith's, lemmatised) require `OPENAI_API_KEY` + `ANTHROPIC_API_KEY` in a local `.env` (the service-role key too, for `pnpm embed`) — none are present in this checkout, and the corpus is currently ad Atticum only, so thematic analytics would be thin regardless. To finish: ingest the fuller corpus (orations/philosophica/further letters), add keys, write the theme-tagging stage in `topics/build.ts`, `pnpm topics`, then re-link the routes in `site-nav.tsx`. Also still pending: `language_groups` (deferred), and re-enabling `AUTH_ENABLED` once the auth UI lands on the new `profiles` table.
