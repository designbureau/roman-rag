...
- ⏭ **Next: write the deferred `profiles`/`handle_new_user` + Cicero `language_groups` + Roman-ensemble `persona_config` migrations; add Phase-3 voices (tiro, atticus, caesar, interpreter, storyteller); then Layer-4 analytics + Layer-5 re-theme.** Re-enable `AUTH_ENABLED` once the auth layer lands.
